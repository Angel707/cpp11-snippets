...
int test_memory_management(void *const start1, void *const start2, const size_t memsize)
{
	...
	MyStaticMemoryAllocator<stype> sma_s1(start1, memsize, "mem1s");
	MyStaticMemoryAllocator<ttype> sma_t1(start1, memsize, "mem1t");
	MyStaticMemoryAllocator<ttype> sma_t2(start2, memsize, "mem2t");
	wait();
	...
	std::cout << "--> " << "change reserved memory for uvec1.." << std::endl;
	std::cout << "now, both vectors does not share the same memory block "
		  << "(cvec1: first 9 bytes, uvec1: all remaining bytes)"  << std::endl;
	const size_t uvec1_memsize2 = memsize - uvec1_memsize1;
	uvec1.reserve(uvec1_memsize2);
	assert(uvec1.capacity() == uvec1_memsize2);
	assert(uvec1.size() == uvec1_added1);
	print_vec(uvec1, "uvec1", "after changing reserved size successfully");
	assert(cvec1.size() == cvec1_added1);
	wait();
